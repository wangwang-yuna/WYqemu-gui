...
class QEMUApp:
    def __init__(self, root):
        ...
        if self.qemu_version:
            self.root.title(f"QEMU GUI (Version: {self.qemu_version})")
        else:
            messagebox.showerror("Error", "QEMU not detected. Please install it in your system.")
            self.install_qemu()

        main_frame = ttk.Frame(self.root, padding="10")
        main_frame.pack(fill=tk.BOTH, expand=True)

        ttk.Label(main_frame, text="QEMU Path:", background="#f0f0f0").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.qemu_path = ttk.Entry(main_frame, width=40)
        self.qemu_path.grid(row=0, column=1, pady=5)
        ttk.Button(main_frame, text="Browse", command=self.load_qemu_path).grid(row=0, column=2, padx=5)

        ttk.Label(main_frame, text="VM Image:", background="#f0f0f0").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.vm_image = ttk.Entry(main_frame, width=40)
        self.vm_image.grid(row=1, column=1, pady=5)
        ttk.Button(main_frame, text="Browse", command=self.load_image).grid(row=1, column=2, padx=5)

        ttk.Label(main_frame, text="Network Configuration:", background="#f0f0f0").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.network_var = ttk.Combobox(main_frame, values=["User Network", "Bridged Network", "Host Network"], state="readonly")
        ...
        ttk.Label(main_frame, text="Bridge NIC:", background="#f0f0f0").grid(row=2, column=2, sticky=tk.W, padx=5)
        ...
        ttk.Label(main_frame, text="CPU Cores:", background="#f0f0f0").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.cpu_cores = ttk.Entry(main_frame, width=10)
        self.cpu_cores.grid(row=3, column=1, sticky=tk.W)

        ttk.Label(main_frame, text="Memory (MB):", background="#f0f0f0").grid(row=3, column=2, sticky=tk.E, padx=5)
        self.memory_size = ttk.Entry(main_frame, width=10)
        self.memory_size.grid(row=3, column=3)

        self.start_button = ttk.Button(main_frame, text="Start VM", command=self.start_vm, width=15)
        self.start_button.grid(row=4, column=0, pady=10)

        self.save_button = ttk.Button(main_frame, text="Save Configuration", command=self.save_config, width=15)
        self.save_button.grid(row=4, column=1, pady=10)

        self.load_button = ttk.Button(main_frame, text="Load Configuration", command=self.load_config, width=15)
        ...
        self.clear_button = ttk.Button(main_frame, text="Clear", command=self.clear_output, width=15)
        self.clear_button.grid(row=6, column=3, pady=5)

        self.status_label = ttk.Label(main_frame, text="Status Information", background="#f0f0f0")
    ...
    def install_qemu(self):
        try:
            ...
            messagebox.showinfo("Information", "QEMU installation complete. Please restart the application.")
            self.root.quit()
        except subprocess.CalledProcessError as e:
            messagebox.showerror("Error", f"Installation failed: {e}")

    def toggle_bridge_nic_input(self, event):
        if self.network_var.get() == "Bridged Network":
    ...
    def load_qemu_path(self):
        qemu_path = filedialog.askdirectory(title="Select QEMU Directory")
        if qemu_path:
    ...
    def start_vm(self):
        if not self.qemu_path.get() or not self.vm_image.get():
            messagebox.showerror("Error", "Please ensure the QEMU path and VM image are filled.")
            return
        ...
        try:
            ...
            messagebox.showerror("Error", "Please enter valid CPU core count and memory size.")
            return

        network_option = self.network_var.get()
        if network_option == "Bridged Network":
            bridge_nic = self.bridge_nic_entry.get().strip()
            if not bridge_nic or bridge_nic == "Select NIC":
                messagebox.showerror("Error", "Please provide a valid network interface card name.")
                return
        ...
        self.log_output(f"Starting: {qemu_command}")
        self.update_status("Starting VM...")

        try:
            subprocess.run(qemu_command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            self.update_status("VM started successfully!")
            self.log_output("VM started successfully!")
        except subprocess.CalledProcessError as e:
            self.update_status("Failed to start VM!")
            error_message = f"Failed to start VM: {e.stderr.decode().strip()}"
    ...
    def load_config(self):
        ...
        if file_path:
            with open(file_path, 'r') as file:
                ...
                self.network_var.set(config.get('network_config', 'User Network'))
